...
# Els consulta ns si fer que fagin prints
...
class ConjuntTrets:
    ...
    # Llista de string
    def _llista_individus(self, tret):
    ...
    def consultar_tret(self,tret):
        if tret not in self.__parelles:
            ...
            ll_id = self._llista_individus(tret)
            for i in sorted(ll_id):
                print(' ', i)

    def distribucio_tret(self, tret, conj_ind):
        if tret not in self.__parelles:
            print('  error')
        else:
            ll_id = self._llista_individus(tret)
            conj_ind.inorder(ll_id)